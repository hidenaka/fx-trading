async function loadData() {
    ...
    try {
        portfolio = await fetchData('portfolio.json');
    } catch (e) {
        console.warn('Portfolio data not available:', e);
    }
    
    try {
        equity = await fetchData('equity_curve.json');
    } catch (e) {
        console.warn('Equity data not available:', e);
    }
    
    try {
        backtest = await fetchLatestBacktest();
    } catch (e) {
        console.warn('Backtest data not available:', e);
    }
    
    if (portfolio || equity || backtest) {
...
async function fetchData(filename) {
    // Try API server first, fallback to local file
    const paths = [
