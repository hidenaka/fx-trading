...
- 必要勝率：55%以上で期待値プラス、目標年6〜10%は勝率57%が前提
...
| ATR(14, 5min)が直近20本平均の3倍超 | エントリー停止＆既ポジ即時決済（フラッシュクラッシュガード） | 1 |

**ペイオフ比は損切り0.5%・利確0.8% で 1.6:1。コスト0.10%込みで勝率55%以上で期待値プラス。**

### フラッシュクラッシュガード（v1.0で追加）

`risk/flash_crash_guard.py`：
- ATR(14, 5min) が直近20本平均の3倍を超えた場合 → 即時全ポジション決済＆当日エントリー停止
- 指値ストップが滑った場合のセーフティ：エントリー -0.7%（損切りラインの1.4倍）にバックストップ成行発注を別途配置
...
- 30件閾値クリアは累積ベース、つまり最初の3-6ヶ月は重みがほぼ動かない
- これは仕様としての意図的な保守姿勢、設計通り
...
- **Rolling 90日 MaxDDの95%分位 ≥ -15%**（v0.9の単純最大DDから信頼区間ベースへ）
- シャープレシオ ≥ 1.0（90日換算）
...
- ATR急増（直近20本平均の3倍超）→ flash_crash_guard 発動
...
- DB破損 → 即停止、バックアップから復元
- Alpaca側で口座制限 → 即停止
- フラッシュクラッシュガード発動回数が直近30日で5回超 → 戦略見直しゲートまで停止
...
1. **数学的期待値の前提**：本仕様は往復コスト0.10%、勝率57%、ペイオフ比1.6:1で年6〜10%目標。実測コストが0.15%超なら期待値マイナス
