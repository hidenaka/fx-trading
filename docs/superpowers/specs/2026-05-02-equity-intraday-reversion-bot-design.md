...
### 期待値の数学（Phase 0で実測値を確定）

ペイオフ比 1.6:1（利確 = 損切り × 1.6、ETFごとのATR比例で実値変動）、往復コスト 0.10% 想定。

**価格ベースの損益分岐勝率（取引コストとポジションサイズによらない値）= 46.15%**
- 計算: `(損切+コスト)/(利確+損切) = (0.5+0.1)/(0.8+0.5) = 0.4615`
- コストが 0.15% に膨らむと損益分岐勝率は 50.0% に上昇 → これが警戒ライン

**ただし、口座対比の実際の期待リターンは以下の要因で複雑：**
- ポジションサイズ上限25%が ATR比例の損切り幅と相互作用（タイトな損切ほど cap が binding になりやすい）
- cap binding 時、1取引の実損失は 0.5% 未満になり、コスト比率が相対的に大きくなる
- 5ETF それぞれで ATR が異なるため、ETF別の実効勝率と頻度の重み付き平均が必要

**簡易な「年取引数 × 勝率 → 年率リターン」表は、上記相互作用を正しく織り込まないと誤った印象を与えるため、本仕様では掲載しない。代わりに：**

→ **Phase 0 のシグナルシミュレータ（`phase0/signal_simulator.py`）が、過去2年データに対して実際のポジションサイズ・cap binding・ETF別ATR・往復コストを正しく再現したバックテストを実行し、現実的な年率期待リターン分布を出力する。**

→ Phase 0 のレポート `calibration_report.md` に「閾値X、想定勝率Y、想定年率Z」の実測ベース表が記載される。これが Paper運用の合格基準・promotion_gate 設定の根拠となる。

**設計ガイドライン：**
- Phase 0 で「想定年率 < +6%」が出た場合 → ポジションサイズ上限を 25% → 50% に緩和（リスク許容度を再確認の上）
- それでも目標未達なら → ペイオフ比を 1.6 → 2.0 に拡大（利確を遠くする、勝率は下がる）
- それでも未達なら → 戦略を根本見直し（mean reversion 自体の劣化を疑う）

**v1.1 までの仕様書では簡易な期待値表を掲載していたが、ポジションcapとATRの相互作用を正しく織り込むのが困難なため、v2.0では Phase 0 実測に委ねる方針に変更した（独立レビュー指摘 D1 に対応）。**
...
| 実測コスト超過 | 0.13%警告/0.18%取引停止 | cost_monitor が判定 | 警告/緊急 |
