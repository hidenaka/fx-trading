...
- v1.1 (2026-05-02): **対象ETFを5本に拡張**（SPY/QQQ→SPY/QQQ/IWM/DIA/XLK）。目標取引頻度を 5〜10/日 = 月100〜200取引に引き上げ。同時保有上限・セクター集中回避ロジック追加。1取引リスクを 0.7%→0.5%、最大ポジションサイズを 50%→25%/銘柄に変更。Promotion Gate の最低取引数を 120→200 に増加

## Goal

米国主要ETF **5本（SPY/QQQ/IWM/DIA/XLK）** を対象に、複数のテクニカルシグナルを組み合わせた短期リバージョン戦略をローカルMacで自動運用し、**スリッページ・スプレッドを正直に織り込んだ上で年6〜10%リターン・最大ドローダウン-20%以内**で長期複利成長を狙う、感情を排した自動売買Botを構築する。各ETFを独立にスキャンし、**1日5〜10回のエントリー機会**を狙う。Paper Trading環境で**最低6ヶ月・累積200取引以上**の検証を経た上で、信頼区間ベースの合格基準を満たした場合のみLive Tradingへ手動承認で移行する。

**期待値前提（v1.1で5ETF / 高頻度向けに再検算）：**
- 往復取引コスト前提：0.10%（スリッページ + スプレッド + SEC/TAF fee）
- 損切り幅：0.5%、利確幅：0.8%、ペイオフ比 1.6:1
- **1取引リスク：0.5%/取引**（v1.0.1の0.7%から下げる、3ポジション同時保有を許容するため）
- **最大ポジションサイズ：1銘柄あたり口座残高の25%**（同時3本保有で総エクスポージャ最大75%）
- **同時保有上限：3ポジション**（5ETF中3本まで）
- **損益分岐勝率：46.15%**（ペイオフ比とコストのみで決まり、頻度に依存しない）
- **年6〜10%リターン達成に必要な勝率：48〜50%**（年200-2400取引、リスク0.5%/取引、複利前提）
- **promotion_gate の勝率基準は55%（安全マージン）**（損益分岐+8pt、v1.0.1の57%は低リスク版前提だったため5ETF版で再調整）
- **取引頻度の懸念**：年2,400取引で往復コスト 0.10% × 2400 = 240bps の手数料負担。コストが想定通りなら問題ないが、実測 0.13% に膨らむと年312bps となり目標達成が困難になる
...
│   │   └── universe.py               # 5ETFのメタデータ・セクター分類・優先順位
│   ├── learning/
...
│   ├── execution/
│   │   ├── intraday_loop.py          # 5分間隔のシグナル評価＆発注（オープン後30分待機、5ETF並列）
│   │   ├── position_manager.py       # 同時保有上限・セクター集中回避・優先順位判定
...
| `data/price_fetcher.py` | 5ETF並列の価格取得・キャッシュ、IEX feed前提 | alpaca-py | 既知期間テスト |
...
| ティッカー | 名称 | 主要構成 | 流動性 | セクター分類 |
|---|---|---|---|---|
| **SPY** | SPDR S&P 500 ETF | 大型株500社 | 最高（出来高8000万株/日） | broad |
| **QQQ** | Invesco QQQ Trust | NASDAQ100（テック比率約50%） | 最高（出来高5000万株/日） | tech-heavy |
| **IWM** | iShares Russell 2000 | 小型株2000社 | 高（出来高3000万株/日） | small-cap |
| **DIA** | SPDR Dow Jones 30 | NYダウ30社 | 高（出来高300万株/日） | broad |
| **XLK** | Technology Select Sector SPDR | テクノロジーセクター | 中（出来高800万株/日） | tech |

各ETFを **独立にスキャン**し、それぞれシグナル評価を行う。

### 入力
- 各ETF（SPY/QQQ/IWM/DIA/XLK）の 1分足・5分足・日足（直近1年分は常時キャッシュ）
- 現在保有ポジション・口座残高
- 現在のシグナル重み（DB から取得、合計1.0、全ETF共通）
- 現在のパラメータ（DB から取得、月次最適化で更新、全ETF共通）
...
**1. トレンドフィルター（必須、ETF別判定）**
- 各ETFについて：当該ETFの日足終値 > 200日移動平均 → そのETFは取引可能
- 各ETFについて：当該ETFの日足終値 < 200日移動平均 → そのETFは当日買い停止（現金待機）
- これにより「QQQは下落トレンド中だがIWMは上昇トレンド中」のような分離判定が可能
- **追加：マクロ防御フィルター** — SPYが200日MA下なら、相関の高い QQQ・XLK・DIA の取引も停止（小型株IWMだけは独立判定可、ただし弱気相場では小型株が先行下落しがちなので参考にする）
...
**統合スコア = Σ(各シグナルのスコア × 重み)**（各ETFごとに独立計算）
- 統合スコア ≥ 0.6 ならエントリー候補（保守側）
- 同一ETFのエントリー後は **5分間のクールダウン**（同方向への即再エントリー禁止）
- 異なるETFは独立にエントリー可能（SPYとIWMが同時にエントリーシグナル発火しても両方OK）

**3. ポジション管理ルール（v1.1で追加、5ETF対応）**

エントリー候補が出ても、以下のチェックを通過しないとエントリーしない：

| ルール | 内容 | 目的 |
|---|---|---|
| **同時保有上限** | 最大3ポジション同時保有 | 過度なリスク集中防止 |
| **セクター集中回避** | {QQQ, XLK} 群から最大1ポジションのみ | 高相関銘柄の重複回避（両方テック比重高） |
| **同銘柄重複禁止** | 既保有のETFには再エントリーしない | サイズ拡大による無自覚なリスク増加防止 |
| **総エクスポージャ上限** | 全ポジション合計で口座残高の75%まで | 最大3ポジ × 25%/銘柄 = 75% |
| **新規エントリー優先順位** | スコア降順、同点時はSPY > QQQ > IWM > DIA > XLK | 流動性順 |

3ポジ既保有時に新たにシグナルが出たら**スコア最下位のポジションを利確/損切り基準到達まで待ってからのみ**入れ替え可能（強制入替はしない）。

**4. ポジションサイズ**
- 1取引のリスク = 口座残高の **0.5%**（v1.0.1の0.7%から下げ、3本同時保有を想定）
- 損切り幅から逆算してドル数量を決定
- **1銘柄あたり最大25%** まで（v1.0.1の50%から下げ、3本×25%=75%が上限）
...
- **Paper運用が累積6ヶ月以上**
- **累積取引回数 ≥ 200件**（v1.1で5ETF/高頻度に対応、v1.0.1の120件から増加）
- 勝率 ≥ 55%（v1.1で5ETFの実質勝率に合わせ調整、損益分岐+8pt）
- プロフィットファクター ≥ 1.3
...
- シャープレシオ ≥ 1.0（90日換算）
- **5ETF個別の勝率も全て45%以上**（特定ETFだけが運で底上げしていないこと、v1.1で追加）
...
**`intraday_runner`：5分間隔（米国市場時間中のみ、5ETF並列処理）**
...
[起動]
  ...
  ↓     - 既保有数 < 3
  ↓     - セクター集中チェック（{QQQ,XLK}重複NG）
  ↓     - マクロ防御フィルター（SPYが200日下なら QQQ/XLK/DIA 取引不可）
  ↓     - クールダウン中でない
  ↓ 11. 通過したものだけ発注、上位から3まで
  ↓ 12. ログ・状態保存
[終了 → heartbeat 完了通知]
```

1回の実行は20〜60秒程度（5ETFのデータ取得・指標計算でAPI呼び出し10〜15回）。Free planのレート制限200req/min から見ても余裕あり。
...
-- ETF別の日次パフォーマンス（v1.1で追加）
CREATE TABLE daily_summary_per_symbol (
  date_et TEXT NOT NULL,
  symbol TEXT NOT NULL,                 -- 'SPY' | 'QQQ' | 'IWM' | 'DIA' | 'XLK'
  trade_count INTEGER,
  win_count INTEGER,
  pnl REAL,
...
**データ：** Alpaca から **SPY/QQQ/IWM/DIA/XLK の5本** について、1分足・5分足・日足を取得、2018〜2024年。各ETFについて独立にシグナル評価＆ポジション管理する。
...
**合格基準（v1.1で5ETF/高頻度版に再調整）：**
- 年率CAGR > **+6%**
- Calmar比 > 1.0（CAGR/MaxDD）
- シャープレシオ > **1.2**
- 最大ドローダウン > -20%
- 勝率 > **55%**（v1.1で5ETFの実質勝率に合わせ調整）
- プロフィットファクター > **1.3**
- 連続マイナス月 < 4
- **月次取引回数 100〜200**（v1.1で5ETF/高頻度に対応）
- **各ETF個別の勝率 > 45%**（特定ETFが他をカバーする偏り防止）
...
**v1.1で再調整：** 6ヶ月、**累積200取引以上**（5ETF対応で取引数増加）
...
- 通貨：USD（Alpaca米国口座）
- **対象ETF：SPY, QQQ, IWM, DIA, XLK（5本、v1.1で拡張）**
- **1取引リスク：0.5%**（v1.0.1の0.7%から下げ、3同時保有想定）
- **最大ポジションサイズ：口座残高の25%/銘柄**（v1.0.1の50%から下げ）
- **同時保有上限：3ポジション**
- **総エクスポージャ上限：75%**（3 × 25%）
- **セクター集中回避：{QQQ, XLK} から最大1本**
- **新規エントリー優先順位：SPY > QQQ > IWM > DIA > XLK**（流動性順）
- 損切り幅：0.5%
- 利確幅：0.8%（ペイオフ比1.6:1）
- 統合スコア閾値：0.6
- 日次損失上限：2%
...
- 月次損失上限：8%
- 累計DD停止：20%
- **トレンドフィルター：各ETFの日足 vs 200日移動平均（ETF別判定）+ SPY基準のマクロ防御フィルター（SPYが200日下なら QQQ/XLK/DIA も停止）**
- オープン後待機時間：30分
- ATR急増判定：直近20本×3倍（ETF別判定）
...
以下は本設計には含めない：
- 個別株の取引（ETFのみ）
- **対象5ETF以外の株式ETF**（VTI, VOO, IVV, VEA等は v1.1スコープ外、将来拡張余地は残す）
- 暗号資産の取引（将来拡張可能性は残す）
- レバレッジETF（TQQQ, SOXL等）の取引
...
- ペアトレード・ロング/ショート同時保有（5ETFの単純買いポジションのみ）
